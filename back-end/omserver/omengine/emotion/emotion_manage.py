import json
import logging
from ..llms.base_llm_driver import LLMDriver
...
class EmotionRecognition():
    ...
    def __init__(self, llm_model_driver: LLMDriver, llm_model_driver_type: str) -> None:
        self.llm_model_driver = llm_model_driver
        self.llm_model_driver_type = llm_model_driver_type

    def recognition(self, your_name: str, query: str) -> str:
        prompt = self.input_prompt.format(input=f"{your_name}说：{query}")+self.output_prompt
        # prompt = self.input_prompt.format(input=f"{query}")+self.output_prompt
        result = self.llm_model_driver.chat(prompt=prompt, type=self.llm_model_driver_type, role_name="", your_name="", query="", kb_id="", short_history=[], long_history="")
...
class EmotionRespond():
    ...
    def respond(self, intent: str, your_name: str, query: str, long_history: str) -> str:
        prompt = self.input_prompt.format(your_name=your_name, query=query, intent=intent, histroy=long_history)+self.output_prompt
        result = self.llm_model_driver.chat(prompt=prompt, type=self.llm_model_driver_type, role_name="", your_name="", query="", kb_id="", short_history=[], long_history="")
